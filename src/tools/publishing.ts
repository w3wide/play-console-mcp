...
export const registerPublishingTools = (server: any) => {
  ...
  server.registerTool(
    'assign_track',
    {
      ...
      inputSchema: {
        ...
        userFraction: z.number().min(0).max(1).optional().describe('Fraction of users who will receive the release (for staged rollouts).'),
      },
    },
  ...
  server.registerTool(
    'commit_edit',
    {
      description: 'Commit an active edit to save and apply all changes. This is when changes go live or to review.',
      inputSchema: {
        packageName: z.string().optional().describe('App package name.'),
        editId: z.string().describe('Active edit ID.'),
      },
